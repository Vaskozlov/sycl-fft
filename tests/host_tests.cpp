...
namespace
{
        ...
        for (std::size_t batch = 0; batch < batch_count; ++batch) {
            ...
            for (std::size_t linear = 0; linear < transform_size; ++linear) {
                ...
                for (std::size_t k = 0; k < n; ++k) {
                    const auto angle = sign * Scalar{2} * pi * static_cast<Scalar>(coordinate * k)
        ...
        std::size_t batch_count, syclfft::direction direction, syclfft::normalization normalization)
    {
        ...
        const auto scale = syclfft::detail::normalization_scale(normalization, direction, n);
    ...
    template <class Scalar>
    void compare(
        const std::vector<std::complex<Scalar>> &actual,
        ...
        const std::string &label)
    {
        ...
        for (std::size_t i = 0; i < actual.size(); ++i) {
            ...
            if (error > bound) {
                throw std::runtime_error(label + ": mismatch at element " + std::to_string(i));
            }
        }
    }
    ...
    template <class Scalar>
    void run_case(
        std::vector<std::size_t> shape, std::size_t batches, syclfft::direction direction,
        syclfft::normalization normalization, syclfft::placement placement)
    {
        ...
        for (std::size_t i = 0; i < count; ++i) {
            input[i] = {
                static_cast<Scalar>((i * 7) % 13) / Scalar{7},
                static_cast<Scalar>((i * 5 + 3) % 11) / Scalar{9}};
        }
        const auto expected = reference(input, shape, batches, direction, normalization);
...
try {
    for (auto normalization :
        ...
        run_case<float>(
            {8}, 2, syclfft::direction::forward, normalization, syclfft::placement::out_of_place);
        run_case<double>(
            {3, 5}, 1, syclfft::direction::backward, normalization, syclfft::placement::in_place);
    }
    run_case<float>(
        {2, 3, 4},
        2,
...

    {
        ...
        for (std::size_t i = 0; i < input.size(); ++i) {
            input[i] = {static_cast<double>(i) / 7.0, static_cast<double>(i % 3) / 5.0};
        }
        auto forward =
            syclfft::host::plan_dft_1d<double>(input.size(), syclfft::direction::forward);
