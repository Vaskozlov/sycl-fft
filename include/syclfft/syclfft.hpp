...
inline void for_each(Group group, Ptr first, Ptr last, Function function) {
  ...
  const auto stride =
      static_cast<std::ptrdiff_t>(group.get_local_linear_range());
...
public:
  ...
  plan(sycl::queue queue, std::vector<std::size_t> lengths,
...
SYCLFFT_SYCL_EXPORT std::vector<provider_status>
query_providers(const sycl::queue &queue);

template <class Scalar>
plan<Scalar>
plan_many_dft(sycl::queue queue, std::span<const std::size_t> lengths,
              std::size_t batch_count, syclfft::direction direction,
              plan_options options = {}) {
  return plan<Scalar>(std::move(queue), {lengths.begin(), lengths.end()},
                      batch_count, direction, options);
}

template <class Scalar>
plan<Scalar>
plan_many_dft(sycl::queue queue, std::initializer_list<std::size_t> lengths,
              std::size_t batch_count, syclfft::direction direction,
              plan_options options = {}) {
  return plan<Scalar>(std::move(queue), {lengths.begin(), lengths.end()},
                      batch_count, direction, options);
...
template <class Scalar>
plan<Scalar> plan_dft_1d(sycl::queue queue, std::size_t n,
...
template <class Scalar>
plan<Scalar> plan_dft_2d(sycl::queue queue, std::size_t n0, std::size_t n1,
                         syclfft::direction direction,
                         plan_options options = {}) {
  return plan_many_dft<Scalar>(std::move(queue), {n0, n1}, 1, direction,
                               options);
}

template <class Scalar>
plan<Scalar> plan_dft_3d(sycl::queue queue, std::size_t n0, std::size_t n1,
                         std::size_t n2, syclfft::direction direction,
                         plan_options options = {}) {
  return plan_many_dft<Scalar>(std::move(queue), {n0, n1, n2}, 1, direction,
                               options);
}
