...
template <class Scalar> struct fftw_traits;

template <> struct fftw_traits<double> {
  using plan_type = fftw_plan;
  using complex_type = fftw_complex;
  static void *allocate(std::size_t bytes) { return fftw_malloc(bytes); }
  static void deallocate(void *ptr) { fftw_free(ptr); }
  static plan_type plan_many(int rank, const int *lengths, int batch,
                             int distance, complex_type *input,
                             complex_type *output, int sign, unsigned flags) {
    return fftw_plan_many_dft(rank, lengths, batch, input, nullptr, 1, distance,
                              output, nullptr, 1, distance, sign, flags);
  }
  static void execute(plan_type plan, complex_type *input,
                      complex_type *output) {
    fftw_execute_dft(plan, input, output);
  }
  static void destroy(plan_type plan) { fftw_destroy_plan(plan); }
  static int alignment(double *ptr) { return fftw_alignment_of(ptr); }
};

template <> struct fftw_traits<float> {
  using plan_type = fftwf_plan;
  using complex_type = fftwf_complex;
  static void *allocate(std::size_t bytes) { return fftwf_malloc(bytes); }
  static void deallocate(void *ptr) { fftwf_free(ptr); }
  static plan_type plan_many(int rank, const int *lengths, int batch,
                             int distance, complex_type *input,
                             complex_type *output, int sign, unsigned flags) {
    return fftwf_plan_many_dft(rank, lengths, batch, input, nullptr, 1,
                               distance, output, nullptr, 1, distance, sign,
                               flags);
  }
  static void execute(plan_type plan, complex_type *input,
                      complex_type *output) {
  ...
  static int alignment(float *ptr) { return fftwf_alignment_of(ptr); }
};

template <class Scalar> class fftw_plan_holder {
...
public:
  explicit fftw_plan_holder(const syclfft_host_plan_config_v1 &config)
    ...
    std::size_t elements = static_cast<std::size_t>(config.batch_count);
    for (std::uint32_t i = 0; i < config.rank; ++i) {
      if (config.lengths[i] == 0 ||
          config.lengths[i] >
              static_cast<std::uint64_t>(std::numeric_limits<int>::max())) {
        throw std::runtime_error(
            "FFTW dimensions must fit in int and be non-zero");
      }
    ...
    sign_ = config.direction < 0 ? FFTW_FORWARD : FFTW_BACKWARD;
    ...
    {
      std::lock_guard lock(fftw_planner_mutex);
      aligned_ = make_plan(flags_);
    }
    if (!aligned_)
      throw std::runtime_error("fftw_plan_many_dft returned null");
    input_alignment_ =
        traits::alignment(static_cast<Scalar *>(planning_input_.get()));
    output_alignment_ =
        traits::alignment(static_cast<Scalar *>(planning_output_alias_));
  }
  ...
  void execute(void *input, void *output) {
    ...
    const bool aligned =
        traits::alignment(reinterpret_cast<Scalar *>(input)) ==
            input_alignment_ &&
        traits::alignment(reinterpret_cast<Scalar *>(output)) ==
            output_alignment_;
...
private:
  struct deleter {
    void operator()(void *ptr) const { traits::deallocate(ptr); }
  };

  native_plan make_plan(unsigned flags) {
    return traits::plan_many(
        rank_, lengths_.data(), batch_, distance_,
        static_cast<native_complex *>(planning_input_.get()),
        static_cast<native_complex *>(planning_output_alias_), sign_, flags);
  }
...
template <class Scalar> struct erased_plan_impl final : erased_plan {
...
void *create_plan(const syclfft_host_plan_config_v1 *config, char *error,
                  size_t capacity) {
  try {
...
void destroy_plan(void *plan) { delete static_cast<erased_plan *>(plan); }

int execute_plan(void *plan, void *input, void *output, char *error,
                 size_t capacity) {
  try {
