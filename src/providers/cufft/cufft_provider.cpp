...
public:
  cufft_plan_impl(sycl::queue queue,
    ...
    check(cufftPlanMany(&handle_, static_cast<int>(config.rank), lengths.data(),
  ...
  sycl::event execute(const void *input, void *output,
                      std::span<const sycl::event> dependencies) {
    const int fft_direction = direction_ == syclfft::direction::forward
                                  ? CUFFT_FORWARD
                                  : CUFFT_INVERSE;
#if defined(SYCLFFT_USE_ADAPTIVECPP)
          ...
          if (double_precision_) {
            ...
            check(
                cufftExecC2C(
                    ...
                    reinterpret_cast<cufftComplex *>(output), fft_direction),
                "cufftExecC2C");
          }
        },
        deps);
    ...
    return queue_.submit([&](sycl::handler &command_group) {
      ...
      command_group.ext_codeplay_enqueue_native_command(
          [=, this](sycl::interop_handle handle) {
            auto stream =
                handle.get_native_queue<sycl::backend::ext_oneapi_cuda>();
            check(cufftSetStream(handle_, stream), "cufftSetStream");
            if (double_precision_) {
              check(cufftExecZ2Z(handle_,
                                 reinterpret_cast<cufftDoubleComplex *>(
...
void *create(sycl::queue &queue,
             const syclfft::detail::cufft_plan_config_v1 &config, char *error,
             std::size_t capacity) {
  try {
...
void destroy(void *plan) { delete static_cast<cufft_plan_impl *>(plan); }

sycl::event execute(void *plan, const void *input, void *output,
                    std::span<const sycl::event> dependencies, char *error,
                    std::size_t capacity) {
  try {
    if (!plan || !input || !output)
      throw std::runtime_error("invalid cuFFT execution argument");
    return static_cast<cufft_plan_impl *>(plan)->execute(input, output,
                                                         dependencies);
  } catch (const std::exception &ex) {
    set_error(error, capacity, ex.what());
    return {};
