...
std::size_t axis_stride(const std::vector<std::size_t> &shape,
                        std::size_t axis) {
  std::size_t result = 1;
...
void validate_pointer(const complex<Scalar> *pointer, const sycl::queue &queue,
                      const char *name) {
  if (!pointer) {
    throw exception(error_code::invalid_pointer,
                    std::string(name) + " must be non-null");
  }
  if (sycl::get_pointer_type(pointer, queue.get_context()) ==
      sycl::usm::alloc::unknown) {
    throw exception(error_code::invalid_pointer,
                    std::string(name) +
                        " is not a USM allocation in the plan context");
  }
}

template <class Scalar>
sycl::event
enqueue_copy_scaled(sycl::queue &queue, const complex<Scalar> *input,
                    complex<Scalar> *output, std::size_t count, Scalar scale,
...
template <class Scalar>
sycl::event enqueue_mixed_axis(sycl::queue &queue, const complex<Scalar> *input,
                               complex<Scalar> *output, std::size_t total,
                               std::size_t length, std::size_t stride,
                               const complex<Scalar> *twiddles,
...
template <class Scalar>
sycl::event
enqueue_radix2_axis(sycl::queue &queue, const complex<Scalar> *input,
                    complex<Scalar> *output, std::size_t total,
                    std::size_t length, std::size_t stride,
...
sycl::event enqueue_bluestein_axis(
    ...
    complex<Scalar> *second, std::size_t total, std::size_t length,
    std::size_t stride, std::size_t convolution_length,
    const complex<Scalar> *chirp, const complex<Scalar> *kernel_spectrum,
    const complex<Scalar> *convolution_twiddles,
    const std::vector<sycl::event> &dependencies) {
  ...
  auto current = queue.submit([&](sycl::handler &handler) {
    ...
    handler.parallel_for(sycl::range<1>{expanded_count}, [=](sycl::id<1> item) {
      ...
      if (coordinate < length) {
        ...
        const auto source =
            outer * length * stride + inner + coordinate * stride;
  ...
  current = enqueue_radix2_axis(queue, first, second, expanded_count,
                                convolution_length, 1, convolution_twiddles,
                                false, {current});
  auto prior = current;
  current = queue.submit([&](sycl::handler &handler) {
    handler.depends_on(prior);
  ...
  current = enqueue_radix2_axis(queue, second, first, expanded_count,
                                convolution_length, 1, convolution_twiddles,
                                true, {current});
  const auto inverse_scale =
      Scalar{1} / static_cast<Scalar>(convolution_length);
  prior = current;
  current = queue.submit([&](sycl::handler &handler) {
    handler.depends_on(prior);
    handler.parallel_for(sycl::range<1>{total}, [=](sycl::id<1> item) {
      ...
      second[linear] =
          first[line * convolution_length + coordinate] *
          complex<Scalar>{chirp[coordinate].real() * inverse_scale,
...
loaded_cufft_provider try_load_cufft(const sycl::queue &queue, bool required,
  ...
  try {
    auto symbol = detail::load_provider_symbol("syclfft_provider_cufft",
                                               "syclfft_get_cufft_provider_v1");
    auto getter =
        reinterpret_cast<detail::get_cufft_provider_v1_fn>(symbol.symbol);
    ...
    if (!api->supports(queue, error.data(), error.size())) {
      ...
      if (required) {
        throw exception(error_code::provider_unavailable,
                        error[0] ? error.data()
                                 : "cuFFT provider rejected the queue");
      }
      return {};
    }
...
public:
  ...
  impl(sycl::queue queue, std::vector<std::size_t> lengths,
       std::size_t batch_count, syclfft::direction dir, plan_options opts)
      : queue_(std::move(queue)), lengths_(std::move(lengths)),
        batch_count_(batch_count), direction_(dir), options_(opts) {
    ...
    if constexpr (std::is_same_v<Scalar, double>) {
      if (!queue_.get_device().has(sycl::aspect::fp64)) {
        throw exception(error_code::unsupported_precision,
    ...
    if (options_.preferred_provider == provider::automatic) {
      ...
      selected_provider_ =
          cufft_.api ? provider::cufft : provider::portable_sycl;
      ...
      host_plan_.emplace(host::plan_many_dft<Scalar>(lengths_, batch_count_,
                                                     direction_, options_));
#else
      throw exception(error_code::provider_unavailable,
    ...
    if (selected_provider_ == provider::cufft) {
      ...
      std::array<char, 1024> error{};
      cufft_handle_ =
          cufft_.api->create(queue_, config, error.data(), error.size());
      if (!cufft_handle_) {
      ...
      try {
        const auto upload_table = [this](
                                      const std::vector<complex_type> &host) {
        ...
        for (std::size_t axis = 0; axis < lengths_.size(); ++axis) {
          ...
          std::vector<complex_type> kernel(convolution_length,
                                           complex_type{Scalar{0}, Scalar{0}});
          ...
          for (std::size_t index = 0; index < length; ++index) {
            ...
            const auto angle =
                -sign * pi * square / static_cast<Scalar>(length);
          ...
          auto convolution_twiddles =
              make_twiddles(convolution_length, Scalar{-1});
          bluestein_tables_.push_back(
              {axis, convolution_length, upload_table(chirp),
               upload_table(kernel), upload_table(convolution_twiddles)});
        }

        scratch_a_ =
            sycl::malloc_device<complex_type>(scratch_elements_, queue_);
        scratch_b_ =
            sycl::malloc_device<complex_type>(scratch_elements_, queue_);
  ...
  sycl::event execute(const complex_type *input, complex_type *output,
    ...
    std::vector<sycl::event> all_dependencies(dependencies.begin(),
                                              dependencies.end());
    if (last_event_)
    ...
    if (selected_provider_ == provider::fftw) {
      ...
      if (sycl::get_pointer_type(input, queue_.get_context()) ==
      ...
      auto completed = queue_.submit(
          [](sycl::handler &handler) { handler.single_task([]{}); });
    ...
    if (selected_provider_ == provider::cufft) {
      std::array<char, 1024> error{};
      auto current =
          cufft_.api->execute(cufft_handle_, input, output, all_dependencies,
      ...
      if (scale != Scalar{1}) {
        ...
        current = enqueue_copy_scaled(queue_, output, output, element_count_,
                                      scale, scale_dependency);
      }
      last_event_ = current;
      return current;
    }
    ...
    for (std::size_t axis = 0; axis < lengths_.size(); ++axis) {
      ...
      if (!factors.empty() || length == 1) {
        const auto table = std::find_if(
            axis_twiddle_tables_.begin(), axis_twiddle_tables_.end(),
        ...
        if (table == axis_twiddle_tables_.end()) {
          throw exception(error_code::invalid_state,
                          "Missing mixed-radix twiddle table");
        }
        current_output = current_input == scratch_a_ ? scratch_b_ : scratch_a_;
        if (is_power_of_two(length)) {
          current = enqueue_radix2_axis(
              queue_, current_input, current_output, element_count_, length,
              stride, table->twiddles, false, axis_dependencies);
        } else {
          current = enqueue_mixed_axis(
              queue_, current_input, current_output, element_count_, length,
              stride, table->twiddles, factors, axis_dependencies);
        }
      } else {
        const auto table =
            std::find_if(bluestein_tables_.begin(), bluestein_tables_.end(),
                         [axis](const bluestein_table &candidate) {
                           return candidate.axis == axis;
                         });
        if (table == bluestein_tables_.end()) {
          throw exception(error_code::invalid_state,
                          "Missing Bluestein plan table");
        }
        ...
        current = enqueue_bluestein_axis(
            queue_, current_input, first, second, element_count_, length,
            stride, table->convolution_length, table->chirp,
            table->kernel_spectrum, table->convolution_twiddles,
            axis_dependencies);
    ...
    current = enqueue_copy_scaled(queue_, current_input, output, element_count_,
                                  scale, final_dependency);
    last_event_ = current;
    return current;
  }
...
template <class Scalar>
plan<Scalar>::plan(sycl::queue queue, std::vector<std::size_t> lengths,
                   std::size_t batch_count, syclfft::direction direction,
                   plan_options options)
    : impl_(std::make_unique<impl>(std::move(queue), std::move(lengths),
                                   batch_count, direction, options)) {}
...
template <class Scalar> sycl::event plan<Scalar>::execute(complex_type *inout) {
  if (!impl_)
    throw exception(error_code::invalid_state,
...
sycl::event plan<Scalar>::execute(complex_type *inout,
  ...
  if (!impl_)
    throw exception(error_code::invalid_state,
                    "Cannot execute a moved-from plan");
  return impl_->execute(inout, inout,
                        std::span<const sycl::event>{&dependency, 1}, true);
}
template <class Scalar>
sycl::event plan<Scalar>::execute(complex_type *inout,
                                  std::span<const sycl::event> dependencies) {
...
template <class Scalar>
sycl::event plan<Scalar>::execute(const complex_type *input,
                                  complex_type *output) {
  if (!impl_)
    throw exception(error_code::invalid_state,
                    "Cannot execute a moved-from plan");
...
sycl::event plan<Scalar>::execute(const complex_type *input,
  ...
  if (!impl_)
    throw exception(error_code::invalid_state,
                    "Cannot execute a moved-from plan");
  return impl_->execute(input, output,
                        std::span<const sycl::event>{&dependency, 1}, false);
}
template <class Scalar>
sycl::event plan<Scalar>::execute(const complex_type *input,
                                  complex_type *output,
...
std::vector<provider_status> query_providers(const sycl::queue &queue) {
  ...
  result.push_back(
      {provider::portable_sycl, "portable_sycl", true, portable_available,
...
#if defined(SYCLFFT_HAS_CUFFT_PROVIDER)
  result.push_back({provider::cufft, "cufft", true, cufft.api != nullptr,
                    cufft.api ? "" : cufft_reason});
#else
  ...
  if (!queue.get_device().is_cpu()) {
    result.push_back(
        {provider::fftw, "fftw", true, false, "queue is not a CPU device"});
  } else {
    try {
...
#else
  result.push_back(
      {provider::fftw, "fftw", false, false, "provider was not built"});
#endif
  result.push_back({provider::rocfft, "rocfft", false, false,
                    "reserved for a future provider"});
  result.push_back({provider::onemkl, "onemkl", false, false,
                    "reserved for a future provider"});
  return result;
}
