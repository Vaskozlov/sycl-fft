...
std::size_t checked_element_count(const std::vector<std::size_t> &lengths,
                                  std::size_t batch_count) {
  if (lengths.empty() || lengths.size() > 3) {
    throw exception(error_code::invalid_argument,
                    "FFT rank must be between 1 and 3");
  }
  if (batch_count == 0) {
    throw exception(error_code::invalid_argument,
                    "FFT batch count must be non-zero");
  }
  std::size_t count = batch_count;
  for (const auto length : lengths) {
    if (length == 0) {
      throw exception(error_code::invalid_argument,
                      "FFT dimensions must be non-zero");
    }
    if (count > std::numeric_limits<std::size_t>::max() / length) {
      throw exception(error_code::invalid_argument,
                      "FFT element count overflows size_t");
    }
    count *= length;
  }
  return count;
...
double normalization_scale(normalization mode, direction dir,
                           std::size_t transform_size) {
  switch (mode) {
  case normalization::none:
    return 1.0;
  case normalization::forward:
    return dir == direction::forward ? 1.0 / static_cast<double>(transform_size)
                                     : 1.0;
  case normalization::backward:
    return dir == direction::backward
               ? 1.0 / static_cast<double>(transform_size)
               : 1.0;
...
loaded_host_provider load_host_provider(const std::string &provider_file) {
  auto loaded =
      load_provider_symbol(provider_file, "syclfft_get_host_provider_v1");
  auto symbol =
      reinterpret_cast<syclfft_get_host_provider_v1_fn>(loaded.symbol);
  const auto *api = symbol();
  if (!api || api->abi_version != SYCLFFT_PROVIDER_ABI_VERSION) {
    throw exception(error_code::plugin_error,
                    "Provider '" + provider_file + "' has an incompatible ABI");
  }
  return {api, std::move(loaded.module)};
}

loaded_provider_symbol load_provider_symbol(const std::string &provider_file,
                                            const std::string &symbol_name) {
  std::string errors;
  for (const auto &directory : provider_search_paths()) {
    const auto path =
        std::filesystem::path(directory) / module_filename(provider_file);
#if defined(_WIN32)
    ...
    void *symbol =
        reinterpret_cast<void *>(GetProcAddress(raw, symbol_name.c_str()));
#else
    void *raw = dlopen(path.c_str(), RTLD_NOW | RTLD_LOCAL);
    if (!raw) {
      const char *message = dlerror();
      errors +=
          path.string() + ": " + (message ? message : "load failed") + "; ";
      continue;
    }
    auto module = std::shared_ptr<void>(raw, [](void *handle) {
      if (handle)
